...
class Trainer:
    ...
    def model_update(self, a1, a2, a3, a4, a5,model_1, model_2, model_3,model_4, model_ft):
        ...
        m1_dict = copy.deepcopy(model_1).state_dict()
        m2_dict = copy.deepcopy(model_2).state_dict()
        m3_dict = copy.deepcopy(model_3).state_dict()
        m4_dict = copy.deepcopy(model_4).state_dict()
        ft_dict = copy.deepcopy(model_ft).state_dict()
    ...
    def _train_online(self):
        ...
        self.mem_n2n_1 = torch.load('./pretrained_models/CLTPMAN/'+str(c[0])+memnum)
        self.mem_n2n_2 = torch.load('./pretrained_models/CLTPMAN/'+str(c[1])+memnum)
        self.mem_n2n_3 = torch.load('./pretrained_models/CLTPMAN/'+str(c[2])+memnum)
        self.mem_n2n_4 = torch.load('./pretrained_models/CLTPMAN/'+str(c[3])+memnum)
        ...
        self.mem_n2n_new.memory_past = torch.zeros(120,48).cuda()
        self.mem_n2n_new.memory_fut  = torch.zeros(120,48).cuda()
        ...
        while not self.test_generator.is_epoch_end():
            ...
            if data is not None:
                ...
                for i in range(len(all_rel)):#len(all_rel)
                    ...
                    if self.cfg.is_update==True:
                        memn2n_copy.train()
                        self.opt = torch.optim.Adam(memn2n_copy.parameters(), lr=lr_set)  # 0.003  /0.02 5
                        self.lr_scheduler = torch.optim.lr_scheduler.StepLR(self.opt, step_size=5, gamma=0.5)

                        epoch = epoch_set# hotel 20
                        # update ft
                        for step in range(epoch):

                            self.opt.zero_grad()
                            output = memn2n_copy(past_rel, obs_traj=past_normalized_one)
                            output = output * scale
                            ## first, calulate ade and fde, then update

                            loss = torch.zeros(1).cuda()
                            loss += mse_loss(output, fut_normalized_one)

                            #loss.requires_grad_(True)

                            loss.backward()
                            torch.nn.utils.clip_grad_norm_(memn2n_copy.parameters(), 1.0, norm_type=2)
                            self.opt.step()
                    ...
                    with torch.no_grad():
                        ...
                        beta = beta_set # 0.28 0.5 1.18 k 0.3
        ...
        plt.plot(x, np.array(f)[:])
        #if is_save:
        # torch.save(aa,'./traj/randomorder_' +str(self.nm) +'ft.pt')
        # torch.save(ade_fde, './curve/10' + str(self.nm) + 'ft.pt')
        plt.savefig(str(self.nm)+'ft.png')
        plt.show()


        #print( 'll', loss_all /samples,samples)
        if self.cfg.is_update:

              print(lr_set,epoch_set,beta_set ,samples, dict_metrics['ade_48s'],dict_metrics['fde_48s'] )
              return loss.item()
        else:
